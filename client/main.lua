	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', _U('texto_tabac'), function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', _U('texto_tabac'), vehicleProps)
		...
		local elements = {
			{label = _U('veh_rumpoo'),  value = 'rumpo6'},
		}
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_tabac', _U('texto_tabac'), amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_tabacjob:hasEnteredMarker', function(zone)
	if zone == 'feuilletabFarm' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac')  then
		CurrentAction     = 'feuilletab_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac')  then
		CurrentAction     = 'tabacsec_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac')  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end

	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac')  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'tabacActions' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') then
		CurrentAction     = 'tabac_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end

	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end

	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') then
...
AddEventHandler('esx_tabacjob:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') then
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString(_U('entreprise_tabac')) --("Entreprise Tabac Pierrot")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end

    if PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', _U('texto_tabac'), vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == _U('texto_tabac') and (GetGameTimer() - GUI.Time) > 150 then
